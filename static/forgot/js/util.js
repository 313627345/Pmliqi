...
define(function (require, exports) {
	var $      = require('jquery'),
	    ...
	    lemon  = require('static/forgot/js/global');
	require('static/forgot/js/jquery.form');
	require('lemon/3rd/plugin');
	...
	exports.validate_conf = function (settings, conf_type) {
		...
		if (typeof conf_type != 'undefined' && conf_type == 'dsk_ajax') {
			settings          = (typeof settings == 'undefined') ? {} : settings;
			var conf_dsk_ajax = {
				submitHandler: function (form) {
					$(form).ajaxSubmit({
			...
			return $.extend(conf_dsk_ajax, settings);
		}
	...
	exports.request_event = function ($this, splash_func) {
		...
		if (str_confirm == 'true') {
			str_confirm = 'You are sure to delete this record?';
		}
		if (str_confirm) {

			layer.confirm(str_confirm, function (index) {
				//do something
				_do_request($this);
				layer.close(index);
			});
	...
	exports.image_popup_show = function (imgSrc, w) {
		if (!imgSrc) {
			exports.splash('error', 'No picture found!');
			return false;
		}
